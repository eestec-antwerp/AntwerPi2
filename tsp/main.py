...
class Field:
	...
	def __init__(self, _input="1_10.txt"):
		...
		#initialize
		maxX, maxY = 100, 100
		turtle.setup(50+Field.scaling_factor*maxX, 50+Field.scaling_factor*maxY)
		...
		for c in self.getCargoList():
			x,y = c.getLocation()
			_x,_y = Field.scaling_factor*x - 5*(self.width/2),Field.scaling_factor*y - 5*(self.height/2)
...
class Cargo:
	...
	def pickup(self):
		...
		Field.alex.speed(0)
		x,y = self.location
		_x,_y = Field.scaling_factor*x - 5*(self.field.width/2),Field.scaling_factor*y - 5*(self.field.height/2)
		Field.alex.goto(_x,_y)
		Field.alex.stamp()
	def dropOff(self):
	...
	def pickupCargo(self):
		...
		if cargo_at_location:
			if cargo_at_location.getWeight() < self.capacity:
				...
				self.cargo[cargo_at_location.getDestination()] = cargo_at_location
				return cargo_at_location
			else:
				print("WARNING: not enough capacity for pickup cargo at location: "+str(self.location))
				return None
		else:
...
if __name__ == "__main__":
	...
	droney = Drone(field, flyspeed="fast") #flyspeed can be "slowest", "slow", "fast", "fastest"

	#######################
	# GAMELOOP (with a random approach)
	#######################

	#preparation (hint: it might be smart to calculate some things before you start flying)
	cargos = list(field.getCargoList())
	index = 0

	while not field.endCondition(level): #argument for endcondition is the scenario: so 1, 2 or 3
		
		#drone coordination (this implementation works, but is very bad. Yours should be better :D)
